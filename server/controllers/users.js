...
export const getUser = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "( /users/id ) something went wrong", error: error });
  }
};
...
export const getFeed = async (req, res) => {
  try {
    ...
    formattedFeed = formattedFeed.map(
      ({
        content,
        user_id,
        id,
    ...
    formattedFeed = await Promise.all(
      formattedFeed.map(async (post) => {
        const { user_id } = post;
        const post_user = await User.findById(user_id);
        const handle = post_user?.user_handle || 'Deleted User';
        const name = post_user?.Name || 'Deleted User';
        const pfp_path = post_user?.picturePath || null;
...
export const getTweet = async (req, res) => {
  try {
    ...
    if (!tweet)
      return res
        .status(204)
        .json({ message: `No tweet with id ${id}`, code: -1 });
    const replies = tweet.replies;

    const repliedTweets = await Promise.all(
...
export const getUserTweets = async (req, res) => {
  try {
    ...
    let user_tweets = await Promise.all(
...
export const followUser = async (req, res) => {
  try {
    ...
    if (id === user_id)
      return res.status(400).json({ status: "Cannot follow self", code: -1 });
...
export const unfollowUser = async (req, res) => {
  try {
    ...
    if (id === user_id)
      return res.status(400).json({ status: "Cannot unfollow self", code: -1 });
...
export const dislikeTweet = async (req, res) => {
  try {
    ...
    if (!tweet)
      return res
        .status(500)
        .json({ type: "Cannot find tweet!", code: -1, liked: null });
